...
## 💡 แนวทางการเขียน Actual Result

### ✅ กรณี Test PASS:
```
Actual Result: 
- ตรงตามที่คาดหวัง
- [อธิบายสิ่งที่เห็นจริง]
- ✓ Check point 1
- ✓ Check point 2
```
...
```
Actual Result:
- แสดง [ผลที่เกิดขึ้นจริง]
- ไม่ตรงกับ Expected Result เพราะ [เหตุผล]
- Bug/Issue ที่พบ: [รายละเอียด]
```
...
✓ Title = "Success"
✓ Message 1: "Thank you for registering with us."
✓ Message 2: "We will send a confirmation to your email soon."
✓ URL changed to: http://localhost:7272/Lab4/Success.html
```

**FAIL Example:**
```
Actual Result:
✗ แสดงหน้า Registration เช่นเดิม (ไม่ redirect)
✗ ไม่มีข้อความ error แสดง
✗ Form ถูก submit แต่ไม่มีการ validate
→ Bug: Validation function ไม่ทำงาน
```

---

## 📝 Tips สำหรับการเขียน Actual Result:

1. **เขียนให้ชัดเจน** - อธิบายสิ่งที่เห็นจริง ไม่ใช่สิ่งที่คาดหวัง
2. **ใส่รายละเอียด** - URL, ข้อความที่แสดง, พฤติกรรมของระบบ
3. **ใช้ Checkmark** - ✓ สำหรับ PASS, ✗ สำหรับ FAIL
4. **Screenshot** - อ้างอิง screenshot หาก test fail
5. **เวลา** - ระบุเวลาที่ test ใช้ (ถ้าจำเป็น)
6. **Browser/Version** - Chrome Version XXX (ถ้าเกี่ยวข้อง)
